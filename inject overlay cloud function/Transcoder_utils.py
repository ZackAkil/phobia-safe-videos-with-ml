...
# gs location of image you want to use as overlay
OVERLAY_IMAGE_URI = 'gs://SOME BUCKET FOR YOUR IMAGE ASSETS/no_see.jpg'
...
def create_fade_overlay_segment(start_time_seconds, end_time_seconds, fade_duration = 0.5):
    ...
    start_time_seconds_fade_start = max(start_time_seconds - fade_duration, 0)
    
    # create fade in animation     
    animation_start = transcoder.Overlay.Animation()
    animation_start.animation_fade = transcoder.Overlay.AnimationFade()
    
    animation_start.animation_fade.start_time_offset = Duration(seconds = int( start_time_seconds_fade_start ), 
                                                                nanos = get_nanos_from_seconds( start_time_seconds_fade_start)  )
    ...
    animation_end.animation_fade.start_time_offset = Duration(seconds = int(end_time_seconds - fade_duration), 
                                                              nanos = get_nanos_from_seconds(end_time_seconds - fade_duration))
    
    animation_end.animation_fade.end_time_offset = Duration(seconds = int(end_time_seconds), 
                                                            nanos = get_nanos_from_seconds(end_time_seconds ))
...
def create_video_overlay_job(input_url, output_folder, output_file_name, overlay_segments):
    ...
    # bitrate_bps=550000
    elementary_video_stream.video_stream = transcoder.VideoStream(frame_rate=30, bitrate_bps=1000000)
